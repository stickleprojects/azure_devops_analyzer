...
Usage format:
python scripts/verify_canary.py --repo-id <repo_id>
...
Canary join query (use this SQL):
...
WHERE r.name = :repo_id LIMIT 1

## Output format

Print results to stdout in this format:
...
Write ONLY the complete, runnable Python source for `scripts/verify_canary.py` as a single code block.
Keep it simple — a `main()` function and `if __name__ == "__main__"` guard. No classes needed.
